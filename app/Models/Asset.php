...
class Asset extends Depreciable
{
   ...
   */
    ...
    {
        if (isset($this->attributes['warranty_months']) && isset($this->attributes['manufacture_date'])) {
            if (is_string($this->attributes['manufacture_date']) || is_string($this->attributes['manufacture_date'])) {
                $manufacture_date = \Carbon\Carbon::parse($this->attributes['manufacture_date']);
            } else {
                $manufacture_date = \Carbon\Carbon::instance($this->attributes['manufacture_date']);
            }
            $manufacture_date->setTime(0, 0, 0);
            return $manufacture_date->addMonths((int) $this->attributes['warranty_months']);
        }
   ...
   */
    ...
    public static function getExpiringWarrantee($days = 30)
    {
        return Asset::where('archived', '=', '0')
            ...
            ->whereNotNull('manufacture_date')
            ->whereNull('deleted_at')
            ->whereRaw(\DB::raw('DATE_ADD(`manufacture_date`,INTERVAL `warranty_months` MONTH) <= DATE(NOW() + INTERVAL '
                                 . $days
                                 . ' DAY) AND DATE_ADD(`manufacture_date`,INTERVAL `warranty_months` MONTH) > NOW()'))
    ...
    public function scopeByFilter($query, $filter)
    {
        return $query->where(function ($query) use ($filter) {
            foreach ($filter as $key => $search_val) {
                ...
                if ($fieldname =='manufacture_date') {
                    $query->where('assets.manufacture_date', 'LIKE', '%'.$search_val.'%');
                }                
